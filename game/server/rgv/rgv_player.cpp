...
//
// valb - TODO: i don't want this, so this will just be set to 1.0 for now until it is refactored completely
#define PLAYER_HULL_REDUCTION 1.0

// This switches between the single primary weapon, and multiple weapons with buckets approach (jdw)
#define	RGV_SINGLE_PRIMARY_WEAPON_MODE	0
...
ConVar rgv_walkspeed( "rgv_walkspeed", "150" );
ConVar rgv_normspeed( "rgv_normspeed", "190" );
ConVar rgv_sprintspeed( "rgv_sprintspeed", "320" );

ConVar rgv_darkness_flashlight_factor ( "rgv_darkness_flashlight_factor", "1" );

#ifdef RGVMP
	#define	RGV_WALK_SPEED 150
	#define	RGV_NORM_SPEED 190
	#define	RGV_SPRINT_SPEED 320
#else
	#define	RGV_WALK_SPEED rgv_walkspeed.GetFloat()
	#define	RGV_NORM_SPEED rgv_normspeed.GetFloat()
	#define	RGV_SPRINT_SPEED rgv_sprintspeed.GetFloat()
#endif
...
bool Flashlight_UseLegacyVersion( void )
{
	...
	if ( g_bCacheLegacyFlashlightStatus )
	{
		...
		g_bUseLegacyFlashlight = ( !Q_strcmp( modDir, "rgv" ) ||
...
void CC_ToggleZoom( void )
{
	...
	if( pPlayer )
	{
		CRGV_Player *pRGVPlayer = dynamic_cast<CRGV_Player*>(pPlayer);

		if( pRGVPlayer && pRGVPlayer->IsSuitEquipped() )
		{
			pRGVPlayer->ToggleZoom();
		}
	}
}
...
#ifndef RGVMP
#ifndef PORTAL
LINK_ENTITY_TO_CLASS( player, CRGV_Player );
#endif
#endif
...
// Global Savedata for RGV player
BEGIN_DATADESC( CRGV_Player )

	DEFINE_FIELD( m_nControlClass, FIELD_INTEGER ),
	DEFINE_EMBEDDED( m_RGVLocal ),
...
IMPLEMENT_SERVERCLASS_ST(CRGV_Player, DT_RGV_Player)
	SendPropDataTable(SENDINFO_DT(m_RGVLocal), &REFERENCE_SEND_TABLE(DT_RGVLocal), SendProxy_SendLocalDataTable),
	SendPropBool( SENDINFO(m_fIsSprinting) ),
END_SEND_TABLE()


void CRGV_Player::Precache( void )
{
	BaseClass::Precache();

	PrecacheScriptSound( "RGVPlayer.SprintNoPower" );
	PrecacheScriptSound( "RGVPlayer.SprintStart" );
	PrecacheScriptSound( "RGVPlayer.UseDeny" );
	PrecacheScriptSound( "RGVPlayer.FlashLightOn" );
	PrecacheScriptSound( "RGVPlayer.FlashLightOff" );
	PrecacheScriptSound( "RGVPlayer.PickupWeapon" );
	PrecacheScriptSound( "RGVPlayer.TrainUse" );
	PrecacheScriptSound( "RGVPlayer.Use" );
	PrecacheScriptSound( "RGVPlayer.BurnPain" );
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CRGV_Player::CheckSuitZoom( void )
{
//#ifndef _XBOX 
...
void CRGV_Player::EquipSuit( bool bPlayEffects )
{
	MDLCACHE_CRITICAL_SECTION();
	BaseClass::EquipSuit();
	
	m_RGVLocal.m_bDisplayReticle = true;
...
void CRGV_Player::RemoveSuit( void )
{
	BaseClass::RemoveSuit();

	m_RGVLocal.m_bDisplayReticle = false;
}

void CRGV_Player::HandleSpeedChanges( void )
{
...
//-----------------------------------------------------------------------------
void CRGV_Player::HandleArmorReduction( void )
{
...
//-----------------------------------------------------------------------------
void CRGV_Player::PreThink(void)
{
	...
	if( m_hLocatorTargetEntity != NULL )
	{
		...
		m_RGVLocal.m_vecLocatorOrigin = m_hLocatorTargetEntity->WorldSpaceCenter();
	}
	else
	{
		m_RGVLocal.m_vecLocatorOrigin = vec3_invalid; // This tells the client we have no locator target.
	}
#endif//RGV_EPISODIC
	...
	if ( IsInAVehicle() )	
	{
		VPROF( "CRGV_Player::PreThink-Vehicle" );
	...
	{
		VPROF( "CRGV_Player::PreThink-Autojump" );
		// check autojump
		Vector vecCheckDir;
	...
	VPROF_SCOPE_BEGIN( "CRGV_Player::PreThink-Speed" );
	HandleSpeedChanges();
#ifdef RGV_EPISODIC
	...
	VPROF_SCOPE_BEGIN( "CRGV_Player::PreThink-ItemPreFrame" );
	ItemPreFrame( );
	VPROF_SCOPE_END();

	VPROF_SCOPE_BEGIN( "CRGV_Player::PreThink-WaterMove" );
	WaterMove();
	VPROF_SCOPE_END();
	...
	VPROF_SCOPE_BEGIN( "CRGV_Player::PreThink-CommanderUpdate" );
	CommanderUpdate();
	VPROF_SCOPE_END();

	// Operate suit accessories and manage power consumption/charge
	VPROF_SCOPE_BEGIN( "CRGV_Player::PreThink-SuitPower_Update" );
	SuitPower_Update();
	VPROF_SCOPE_END();

	// checks if new client data (for HUD and view control) needs to be sent to the client
	VPROF_SCOPE_BEGIN( "CRGV_Player::PreThink-UpdateClientData" );
	UpdateClientData();
	VPROF_SCOPE_END();
	
	VPROF_SCOPE_BEGIN( "CRGV_Player::PreThink-CheckTimeBasedDamage" );
	CheckTimeBasedDamage();
	VPROF_SCOPE_END();

	VPROF_SCOPE_BEGIN( "CRGV_Player::PreThink-CheckSuitUpdate" );
	CheckSuitUpdate();
	VPROF_SCOPE_END();

	VPROF_SCOPE_BEGIN( "CRGV_Player::PreThink-CheckSuitZoom" );
	CheckSuitZoom();
	VPROF_SCOPE_END();
...
void CRGV_Player::PostThink( void )
{
	BaseClass::PostThink();
...
void CRGV_Player::StartAdmireGlovesAnimation( void )
{
	MDLCACHE_CRITICAL_SECTION();
...
void CRGV_Player::HandleAdmireGlovesAnimation( void )
{
...
#define RGVPLAYER_RELOADGAME_ATTACK_DELAY 1.0f

void CRGV_Player::Activate( void )
{
	BaseClass::Activate();
	InitSprinting();
	...
	if ( GetActiveWeapon() )
	{
		...
		if ( flRemaining < RGVPLAYER_RELOADGAME_ATTACK_DELAY )
		{
			GetActiveWeapon()->m_flNextPrimaryAttack = gpGlobals->curtime + RGVPLAYER_RELOADGAME_ATTACK_DELAY;
		}
		...
		if ( flRemaining < RGVPLAYER_RELOADGAME_ATTACK_DELAY )
		{
			GetActiveWeapon()->m_flNextSecondaryAttack = gpGlobals->curtime + RGVPLAYER_RELOADGAME_ATTACK_DELAY;
		}
	}
...
// Output  :
//------------------------------------------------------------------------------
Class_T  CRGV_Player::Classify ( void )
{
...
//-----------------------------------------------------------------------------
bool CRGV_Player::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* sourceEnt)
{
...
void CRGV_Player::PlayerRunCommand(CUserCmd *ucmd, IMoveHelper *moveHelper)
{
	// Handle FL_FROZEN.
	if ( m_afPhysicsFlags & PFLAG_ONBARNACLE )
	{
...
//-----------------------------------------------------------------------------
// Purpose: Sets RGV specific defaults.
//-----------------------------------------------------------------------------
void CRGV_Player::Spawn(void)
{
...
#ifdef RGV_EPISODIC
	m_RGVLocal.m_flFlashBattery = 100.0f;
#endif 
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CRGV_Player::UpdateLocatorPosition( const Vector &vecPosition )
{
#ifdef RGV_EPISODIC
	m_RGVLocal.m_vecLocatorOrigin = vecPosition;
#endif//RGV_EPISODIC 
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CRGV_Player::InitSprinting( void )
{
	StopSprinting();
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CRGV_Player::StartAutoSprint() 
{
	if( IsSprinting() )
	{
		StopSprinting();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CRGV_Player::StartSprinting( void )
{
	if( m_RGVLocal.m_flSuitPower < 10 )
	{
		...
		if ( m_afButtonPressed & IN_SPEED )
		{
			...
			EmitSound( filter, entindex(), "RGVPlayer.SprintNoPower" );
		}
		return;
	}
	...
	CPASAttenuationFilter filter( this );
	filter.UsePredictionRules();
	EmitSound( filter, entindex(), "RGVPlayer.SprintStart" );

	SetMaxSpeed( RGV_SPRINT_SPEED );
	m_fIsSprinting = true;
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CRGV_Player::StopSprinting( void )
{
	if ( m_RGVLocal.m_bitsActiveDevices & SuitDeviceSprint.GetDeviceID() )
	{
	...
	if( IsSuitEquipped() )
	{
		SetMaxSpeed( RGV_NORM_SPEED );
	}
	else
	{
		SetMaxSpeed( RGV_WALK_SPEED );
	}
...
//-----------------------------------------------------------------------------
void CRGV_Player::EnableSprint( bool bEnable )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CRGV_Player::StartWalking( void )
{
	SetMaxSpeed( RGV_WALK_SPEED );
	m_fIsWalking = true;
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CRGV_Player::StopWalking( void )
{
	SetMaxSpeed( RGV_NORM_SPEED );
	m_fIsWalking = false;
}
...
//-----------------------------------------------------------------------------
bool CRGV_Player::CanZoom( CBaseEntity *pRequester )
{
	if ( IsZooming() )
		return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CRGV_Player::ToggleZoom(void)
{
	if( IsZooming() )
	{
		StopZooming();
...
//-----------------------------------------------------------------------------
void CRGV_Player::StartZooming( void )
{
	int iFOV = 25;
	if ( SetFOV( this, iFOV, 0.4f ) )
	{
		m_RGVLocal.m_bZooming = true;
	}
}
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CRGV_Player::IsZooming( void )
{
...
public:
	...
	{
		CRGV_Player *pPlayer = (CRGV_Player *)pObject->GetGameData();
		if ( pPlayer )
		{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CRGV_Player::InitVCollision( const Vector &vecAbsOrigin, const Vector &vecAbsVelocity )
{
	BaseClass::InitVCollision( vecAbsOrigin, vecAbsVelocity );

	// Setup the RGV specific callback.
...
bool CRGV_Player::CommanderFindGoal( commandgoal_t *pGoal )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CAI_BaseNPC *CRGV_Player::GetSquadCommandRepresentative()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CRGV_Player::GetNumSquadCommandables()
{
	AISquadIter_t iter;
	int c = 0;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CRGV_Player::GetNumSquadCommandableMedics()
{
	AISquadIter_t iter;
	int c = 0;
	for ( CAI_BaseNPC *pAllyNpc = m_pPlayerAISquad->GetFirstMember(&iter); pAllyNpc; pAllyNpc = m_pPlayerAISquad->GetNextMember(&iter) )
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CRGV_Player::CommanderUpdate()
{
	...
	if ( pCommandRepresentative )
	{
		...
		m_RGVLocal.m_iSquadMemberCount = GetNumSquadCommandables();
		m_RGVLocal.m_iSquadMedicCount = GetNumSquadCommandableMedics();
		m_RGVLocal.m_fSquadInFollowMode = bFollowMode;
	...
	{
		m_RGVLocal.m_iSquadMemberCount = 0;
		m_RGVLocal.m_iSquadMedicCount = 0;
		m_RGVLocal.m_fSquadInFollowMode = true;
	}
...
//----------------------------------------------------------------------------- 
bool CRGV_Player::CommanderExecuteOne( CAI_BaseNPC *pNpc, const commandgoal_t &goal, CAI_BaseNPC **Allies, int numAllies )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CRGV_Player::CommanderExecute( CommanderCommand_t command )
{
	CAI_BaseNPC *pPlayerSquadLeader = GetSquadCommandRepresentative();

	if ( !pPlayerSquadLeader )
	{
		EmitSound( "RGVPlayer.UseDeny" );
		return;
	}
...
//-----------------------------------------------------------------------------
void CRGV_Player::CommanderMode()
{
...
// Input  : iImpulse - 
//-----------------------------------------------------------------------------
void CRGV_Player::CheatImpulseCommands( int iImpulse )
{
	switch( iImpulse )
	{
	case 50:
...
// Purpose: 
//-----------------------------------------------------------------------------
void CRGV_Player::SetupVisibility( CBaseEntity *pViewEntity, unsigned char *pvs, int pvssize )
{
...
void CRGV_Player::SuitPower_Update( void )
{
	...
	else if( m_RGVLocal.m_bitsActiveDevices )
	{
		...
		if ( Flashlight_UseLegacyVersion() )
		{
			...
			if( m_RGVLocal.m_flSuitPower < 4.8f && FlashlightIsOn() )
			{
#ifndef RGVMP
				FlashlightTurnOff();
#endif
...
//-----------------------------------------------------------------------------
void CRGV_Player::SuitPower_Initialize( void )
{
	m_RGVLocal.m_bitsActiveDevices = 0x00000000;
	m_RGVLocal.m_flSuitPower = 100.0;
...
//-----------------------------------------------------------------------------
bool CRGV_Player::SuitPower_Drain( float flPower )
{
	// Suitpower cheat on?
	if ( sv_infinite_aux_power.GetBool() )
		return true;

	m_RGVLocal.m_flSuitPower -= flPower;

	if( m_RGVLocal.m_flSuitPower < 0.0 )
	{
		// Power is depleted!
		// Clamp and fail
		m_RGVLocal.m_flSuitPower = 0.0;
		return false;
	}
...
//-----------------------------------------------------------------------------
void CRGV_Player::SuitPower_Charge( float flPower )
{
	m_RGVLocal.m_flSuitPower += flPower;

	if( m_RGVLocal.m_flSuitPower > 100.0 )
	{
		// Full charge, clamp.
		m_RGVLocal.m_flSuitPower = 100.0;
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CRGV_Player::SuitPower_IsDeviceActive( const CSuitPowerDevice &device )
{
	return (m_RGVLocal.m_bitsActiveDevices & device.GetDeviceID()) != 0;
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CRGV_Player::SuitPower_AddDevice( const CSuitPowerDevice &device )
{
	// Make sure this device is NOT active!!
	if( m_RGVLocal.m_bitsActiveDevices & device.GetDeviceID() )
		return false;

	if( !IsSuitEquipped() )
		return false;

	m_RGVLocal.m_bitsActiveDevices |= device.GetDeviceID();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CRGV_Player::SuitPower_RemoveDevice( const CSuitPowerDevice &device )
{
	...
	if( m_RGVLocal.m_bitsActiveDevices == 0x00000000 )
	{
...
bool CRGV_Player::SuitPower_ShouldRecharge( void )
{
	...
	if( m_RGVLocal.m_flSuitPower >= 100.0f )
		return false; 
...
bool CRGV_Player::ApplyBattery( float powerMultiplier )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CRGV_Player::FlashlightIsOn( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CRGV_Player::FlashlightTurnOn( void )
{
	...
	EmitSound( "RGVPlayer.FlashLightOn" );

	variant_t flashlighton;
	flashlighton.SetFloat( m_RGVLocal.m_flSuitPower / 100.0f );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CRGV_Player::FlashlightTurnOff( void )
{
	if ( Flashlight_UseLegacyVersion() )
	{
	...
	EmitSound( "RGVPlayer.FlashLightOff" );

	variant_t flashlightoff;
	flashlightoff.SetFloat( m_RGVLocal.m_flSuitPower / 100.0f );
...
bool CRGV_Player::IsIlluminatedByFlashlight( CBaseEntity *pEntity, float *flReturnDot )
{
	if( !FlashlightIsOn() )
		return false;
...
//-----------------------------------------------------------------------------
void CRGV_Player::CheckFlashlight( void )
{
	if ( !FlashlightIsOn() )
		return;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CRGV_Player::SetPlayerUnderwater( bool state )
{
	if ( state )
	{
...
//-----------------------------------------------------------------------------
bool CRGV_Player::PassesDamageFilter( const CTakeDamageInfo &info )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CRGV_Player::SetFlashlightEnabled( bool bState )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CRGV_Player::InputDisableFlashlight( inputdata_t &inputdata )
{
	if( FlashlightIsOn() )
		FlashlightTurnOff();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CRGV_Player::InputEnableFlashlight( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
void CRGV_Player::InputIgnoreFallDamage( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
void CRGV_Player::InputIgnoreFallDamageWithoutReset( inputdata_t &inputdata )
{
	float timeToIgnore = inputdata.value.Float();
...
//-----------------------------------------------------------------------------
void CRGV_Player::OnSquadMemberKilled( inputdata_t &data )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CRGV_Player::NotifyFriendsOfDamage( CBaseEntity *pAttackerEntity )
{
...
int	CRGV_Player::OnTakeDamage( const CTakeDamageInfo &info )
{
...
// Input  : &info - 
//-----------------------------------------------------------------------------
int CRGV_Player::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	// Drown
	...
	{
		EmitSound( "RGVPlayer.BurnPain" );
	}


	if( (info.GetDamageType() & DMG_SLASH) && rgv_episodic.GetBool() )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CRGV_Player::OnDamagedByExplosion( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CRGV_Player::ShouldShootMissTarget( CBaseCombatCharacter *pAttacker )
{
...
//-----------------------------------------------------------------------------
void CRGV_Player::CombineBallSocketed( CPropCombineBall *pCombineBall )
{
#ifdef RGV_EPISODIC
	CNPC_Alyx *pAlyx = CNPC_Alyx::GetAlyx();
	if ( pAlyx )
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CRGV_Player::Event_KilledOther( CBaseEntity *pVictim, const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CRGV_Player::Event_Killed( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CRGV_Player::NotifyScriptsOfDeath( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CRGV_Player::GetAutoaimVector( autoaim_params_t &params )
{
	...
	if ( IsX360() )
	{
		...
		if( params.m_fScale != AUTOAIM_SCALE_DIRECT_ONLY )
		{
			...
			{
				// Turn on sticky.
				m_RGVLocal.m_bStickyAutoAim = true;
			...
			else if( !params.m_hAutoAimEntity )
			{
				...
				m_RGVLocal.m_bStickyAutoAim = false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CRGV_Player::ShouldKeepLockedAutoaimTarget( EHANDLE hLockedTarget )
{
	Vector vecLooking;
	Vector vecToTarget;
...
// Output : int
//-----------------------------------------------------------------------------
int CRGV_Player::GiveAmmo( int nCount, int nAmmoIndex, bool bSuppressSound)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CRGV_Player::Weapon_CanUse( CBaseCombatWeapon *pWeapon )
{
#ifndef RGVMP	
...
// Input  : *pWeapon - 
//-----------------------------------------------------------------------------
void CRGV_Player::Weapon_Equip( CBaseCombatWeapon *pWeapon )
{
#if	RGV_SINGLE_PRIMARY_WEAPON_MODE
	...
	{
		m_RGVLocal.m_bWeaponLowered = false;
	}
...
//-----------------------------------------------------------------------------
bool CRGV_Player::BumpWeapon( CBaseCombatWeapon *pWeapon )
{
	...
	else 
	{
		...
		EmitSound( "RGVPlayer.PickupWeapon" );
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CRGV_Player::ClientCommand( const CCommand &args )
{
#if	RGV_SINGLE_PRIMARY_WEAPON_MODE
...
//-----------------------------------------------------------------------------
void CRGV_Player::PlayerUse ( void )
{
...
//-----------------------------------------------------------------------------
void CRGV_Player::UpdateWeaponPosture( void )
{
	...
	if ( pWeapon && m_LowerWeaponTimer.Expired() && pWeapon->CanLower() )
	{
		...
		VPROF( "CRGV_Player::UpdateWeaponPosture-CheckLower" );
	...
	if( g_pGameRules->GetAutoAimMode() != AUTOAIM_NONE )
	{
		if( !pWeapon )
		{
			...
			m_RGVLocal.m_bWeaponLowered = true;
			return;
		}
		else
		{
			if( !pWeapon->CanLower() && m_RGVLocal.m_bWeaponLowered )
				m_RGVLocal.m_bWeaponLowered = false;
		}
		...
		m_RGVLocal.m_hAutoAimTarget.Set( params.m_hAutoAimEntity );
		m_RGVLocal.m_vecAutoAimPoint.Set( params.m_vecAutoAimPoint );
		m_RGVLocal.m_bAutoAimTarget = ( params.m_bAutoAimAssisting || params.m_bOnTargetNatural );
		return;
	}
	else
	{
		// Make sure there's no residual autoaim target if the user changes the xbox_aiming convar on the fly.
		m_RGVLocal.m_hAutoAimTarget.Set(NULL);
	}
}
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CRGV_Player::Weapon_Lower( void )
{
	VPROF( "CRGV_Player::Weapon_Lower" );
	// Already lowered?
	if ( m_RGVLocal.m_bWeaponLowered )
		return true;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CRGV_Player::Weapon_Ready( void )
{
	VPROF( "CRGV_Player::Weapon_Ready" );

	// Already ready?
	if ( m_RGVLocal.m_bWeaponLowered == false )
		return true;
...
// Input  : pWeapon - 
//-----------------------------------------------------------------------------
bool CRGV_Player::Weapon_CanSwitchTo( CBaseCombatWeapon *pWeapon )
{
...
void CRGV_Player::PickupObject( CBaseEntity *pObject, bool bLimitMassAndSize )
{
...
// Output : CBaseEntity
//-----------------------------------------------------------------------------
bool CRGV_Player::IsHoldingEntity( CBaseEntity *pEnt )
{
	return PlayerPickupControllerIsHoldingEntity( m_hUseEntity, pEnt );
}

float CRGV_Player::GetHeldObjectMass( IPhysicsObject *pHeldObject )
{
...
//-----------------------------------------------------------------------------
void CRGV_Player::ForceDropOfCarriedPhysObjects( CBaseEntity *pOnlyIfHoldingThis )
{
	if ( PhysIsInCallback() )
	{
		variant_t value;
...
void CRGV_Player::InputForceDropPhysObjects( inputdata_t &data )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CRGV_Player::UpdateClientData( void )
{
	...
	if ( Flashlight_UseLegacyVersion() == false )
	{
		...
		{
			m_RGVLocal.m_flFlashBattery -= FLASH_DRAIN_TIME * gpGlobals->frametime;
			if ( m_RGVLocal.m_flFlashBattery < 0.0f )
			{
				FlashlightTurnOff();
				m_RGVLocal.m_flFlashBattery = 0.0f;
			}
		}
		else
		{
			m_RGVLocal.m_flFlashBattery += FLASH_CHARGE_TIME * gpGlobals->frametime;
			if ( m_RGVLocal.m_flFlashBattery > 100.0f )
			{
				m_RGVLocal.m_flFlashBattery = 100.0f;
			}
		}
	...
	{
		m_RGVLocal.m_flFlashBattery = -1.0f;
	}
#endif // RGV_EPISODIC
...
//---------------------------------------------------------
//---------------------------------------------------------
Vector CRGV_Player::EyeDirection2D( void )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
Vector CRGV_Player::EyeDirection3D( void )
{
	Vector vecForward;
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CRGV_Player::Weapon_Switch( CBaseCombatWeapon *pWeapon, int viewmodelindex )
{
	MDLCACHE_CRITICAL_SECTION();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
WeaponProficiency_t CRGV_Player::CalcWeaponProficiency( CBaseCombatWeapon *pWeapon )
{
...
bool CRGV_Player::TestHitboxes( const Ray_t &ray, unsigned int fContentsMask, trace_t& tr )
{
...
// bullet impacts.
//---------------------------------------------------------
void CRGV_Player::DrawDebugGeometryOverlays(void) 
{
...
void CRGV_Player::ExitLadder()
{
	...
	// Remove from ladder
	m_RGVLocal.m_hLadder.Set( NULL );
}


surfacedata_t *CRGV_Player::GetLadderSurface( const Vector &origin )
{
	extern const char *FuncLadder_GetSurfaceprops(CBaseEntity *pLadderEntity);

	CBaseEntity *pLadder = m_RGVLocal.m_hLadder.Get();
	if ( pLadder )
	{
...
//-----------------------------------------------------------------------------
void CRGV_Player::PlayUseDenySound()
{
	m_bPlayUseDenySound = true;
}


void CRGV_Player::ItemPostFrame()
{
...
void CRGV_Player::StartWaterDeathSounds( void )
{
	CPASAttenuationFilter filter( this );
...
void CRGV_Player::StopWaterDeathSounds( void )
{
	if ( m_sndLeeches )
	{
...
// 
//-----------------------------------------------------------------------------
void CRGV_Player::MissedAR2AltFire()
{
...
//
//-----------------------------------------------------------------------------
void CRGV_Player::DisplayLadderHudHint()
{
#if !defined( CLIENT_DLL )
...
// Shuts down sounds
//-----------------------------------------------------------------------------
void CRGV_Player::StopLoopingSounds( void )
{
	if ( m_sndLeeches != NULL )
	{
...
//-----------------------------------------------------------------------------
void CRGV_Player::ModifyOrAppendPlayerCriteria( AI_CriteriaSet& set )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
const impactdamagetable_t &CRGV_Player::GetPhysicsImpactDamageTable()
{
...
CLogicPlayerProxy *CRGV_Player::GetPlayerProxy( void )
{
...
void CRGV_Player::FirePlayerProxyOutput( const char *pszOutputName, variant_t variant, CBaseEntity *pActivator, CBaseEntity *pCaller )
{
	if ( GetPlayerProxy() == NULL )
		return;
...
void CLogicPlayerProxy::InputSetFlashlightSlowDrain( inputdata_t &inputdata )
{
	...
	CRGV_Player *pPlayer = dynamic_cast<CRGV_Player*>(m_hPlayer.Get());

	if( pPlayer )
		pPlayer->SetFlashlightPowerDrainScale( rgv_darkness_flashlight_factor.GetFloat() );
}
